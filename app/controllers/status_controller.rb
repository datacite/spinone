class StatusController < ApplicationController
  def index
    Status.create(current_version: Spinone::VERSION) if Rails.env == "development" || Status.count == 0
    ...
    if current_user.try(:is_admin?) && @current_status.outdated_version?
      flash.now[:alert] = "Your Spinone software is outdated, please install <a href='#{ENV['GITHUB_URL']}/releases'>version #{@current_status.current_version}</a>.".html_safe
      @flash = flash
    end
  end
